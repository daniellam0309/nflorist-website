...
const T = {
  zh: {
    ...
    albums: {
      ...
      mothersday2026: ["2026年母親節花禮", "以溫柔花色與細緻花材，為母親節準備充滿心意的節日花禮。"],
    },
  ...
  en: {
    ...
    albums: {
      ...
      mothersday2026: ["Mother’s Day 2026", "A seasonal floral gift collection designed with soft palettes and heartfelt details for Mother’s Day."],
    },
...
const albumSpecs = [
  ...
  ["mothersday2026", (f) => range(f, "mothersday", 1, 13)],
];
...
const AmbientGlassBloom = () => {
  ...
  return <div className="pointer-events-none fixed inset-0 z-[2] overflow-hidden" aria-hidden="true"><div className="absolute -left-24 top-[10%] h-80 w-80 rounded-full bg-[#c7b98b]/10 blur-3xl" /><div className="absolute right-[4%] top-[8%] h-96 w-96 rounded-full bg-[#6f745f]/10 blur-3xl" /><div className="absolute left-[36%] bottom-[4%] h-[28rem] w-[28rem] rounded-full bg-[#c7b98b]/[0.06] blur-3xl" />{petals.map((p, i) => <motion.div key={`glass-petal-${i}`} className="absolute top-[-24vh] rounded-[68%_32%_70%_30%] border border-white/10 blur-[0.6px]" style={{ left: p.left, width: p.size, height: p.size * 1.45, rotate: `${p.rotate}deg`, background: "linear-gradient(145deg, rgba(244,239,230,0.07) 0%, rgba(199,185,139,0.12) 42%, rgba(111,116,95,0.08) 100%)", boxShadow: "0 0 46px rgba(199,185,139,0.045), inset 0 0 18px rgba(255,255,255,0.04)" }} animate={{ y: ["-24vh", "30vh", "72vh", "126vh"], x: [0, p.drift, -p.drift * 0.7, p.drift * 0.35], rotate: [p.rotate, p.rotate + 10, p.rotate - 12, p.rotate + 4], opacity: [0, 0.18, 0.28, 0.1, 0], scale: [0.88, 1.02, 1.07, 0.96], filter: ["brightness(0.92)", "brightness(1.08)", "brightness(0.96)", "brightness(1.06)", "brightness(0.92)"] }} transition={{ duration: p.dur, delay: p.delay, repeat: Infinity, ease: "easeInOut" }} />)}{glints.map((g, i) => <motion.span key={`glass-glint-${i}`} className="absolute rounded-full bg-[#f4efe6]/30 blur-[0.8px]" style={{ left: g.left, top: g.top, width: g.size, height: g.size }} animate={{ opacity: [0, 0.45, 0.12, 0.5, 0], scale: [0.7, 1.15, 0.85, 1.25, 0.7] }} transition={{ duration: g.dur, delay: g.delay, repeat: Infinity, ease: "easeInOut" }} />)}</div>;
};
...
export function validateWebsiteData() {
  ...
  if (holidayPath !== "/mother'sday") errors.push("Holiday path changed unexpectedly.");
...
export default function NfloristWebsiteConcept() {
  ...
  const isHolidayPage = typeof window !== "undefined" && window.location.pathname.toLowerCase().includes("mother");
  const t = T[lang];
  ...
  const mothersDayAlbum = albums.find((x) => x.id === "mothersday2026") || albums[albums.length - 1];
